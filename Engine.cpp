    ...
    : primitives(), player(windowTitle, screenWidth, screenHeight), isFullscreen(false), blockSpawnInterval(2.0f) // Inicjalizacja blockSpawnInterval
{
    ...
    if (!font.loadFromFile("fonts/ARCADECLASSIC.ttf"))
    {
        ...
        if (!backgroundTexture.loadFromFile("textures/tlo.png")) {
            cerr << "B³¹d ³adowania tekstury t³a" << endl;
        }
        if (!mainBlockTexture.loadFromFile("textures/main.png")) {
            cerr << "B³¹d ³adowania tekstury bloku glownego" << endl;
        }
    ...
    background.setScale(static_cast<float>(screenWidth) / backgroundTexture.getSize().x,
        static_cast<float>(screenHeight) / backgroundTexture.getSize().y);
    main_block.setTexture(mainBlockTexture);
    main_block.setScale(1.0f,2.0f);
    main_block.setPosition(0, 830);
